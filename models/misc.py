...
def collect_distributed_list(data, tmp_path, save_path):
    import json
    ...
    if not is_dist_avail_and_initialized():
        ...
        with open(os.path.join(tmp_path, f'rat_{get_rank()}.json'), 'w') as f:
            f.write(json.dumps(data))
        torch.distributed.barrier()
        ...
        if is_main_process():
            ...
            with open(save_path, 'w') as f:
                f.write(json.dumps(collected_rationale))
            data = collected_rationale
