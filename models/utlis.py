...
def plug_and_play_fast_ranking(
    ...
    selected_idx = scores.topk(3, dim=-1)[1]
    return selected_idx
...
def PlugAndPlayContrastiveDecodingOneStepFast(model, input_ids, prefix_len, beam_width, alpha, beta,
                                              ...
                                              input_ids_for_class=None, img_len=1, step=0):  # , add_token_level_score=False):
    '''
    ...
    if first_step:
        inputs = {'inputs_embeds': input_ids, 'past_key_values': past_key_values, 'use_cache': True,
                  'output_hidden_states': True}
        output = model(**inputs)
        past_key_values = output.past_key_values
        last_hidden_states = output.hidden_states[-1]  # [B, S, E]
    ...
    output = model(
        input_ids=top_k_ids.view(-1, 1),
        attention_mask=torch.ones_like(top_k_ids.view(-1, 1).repeat(1, prefix_len + 1 + step)),
        past_key_values=past_key_values,
        output_hidden_states=True,
        use_cache=True,
    )
    ...
    if input_ids_for_class is not None:
    ...
    for one_input_id in input_ids_for_class_:
        try:
            one_text = simctg_tokenizer.decode(one_input_id[prefix_len - img_len:][-clip_text_max_len:])
        except:
            one_text = ''
        # we only consider the class score of the generated text continuation
        batch_text_list.append(one_text)
    batch_score = clip.compute_image_text_similarity_via_raw_text(image_embeds, batch_text_list, context_hidden.device)

    selected_topk_idx = plug_and_play_fast_ranking(
        context_hidden,
        next_hidden,
        top_k_ids,
        top_k_probs,
    ...
    selected_idx = -1
    # prepare for the next step
    for i in selected_topk_idx[0]:
        try:
            next_id = top_k_ids[range(len(top_k_ids)), i].unsqueeze(-1)
            if input_ids_for_class is not None:
                simctg_tokenizer.decode(torch.cat([input_ids_for_class, next_id], dim=-1)[0])
            else:
                simctg_tokenizer.decode(next_id[0])
            # simctg_tokenizer.decode(torch.cat([input_ids_for_class, next_id], dim=-1)[0])
            selected_idx = i
            break
        except:
            continue
    if selected_idx == -1:
        print("All Protection Protocol Fail")
        next_id = torch.ones_like(selected_topk_idx[0][0]).unsqueeze(-1).unsqueeze(-1)
