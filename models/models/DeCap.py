...
class DeCap(BaseModel):
    ...
    def __init__(self, clip_model, llm, tokenizer, args=None, prefix_size: int = 512):
        ...
        memory_bank = torch.load(args.memory_path, map_location='cpu')
    ...
    def generate(self, img, gen_strategy):
        ...
        # fetch memory bank
        if self.memory_bank.max() > 0:
            # valid mb
            sim = cls_features @ self.memory_bank.T
            sim = (sim * 100).softmax(dim=-1)
            prefix_embedding = sim @ self.memory_bank
            prefix_embedding /= prefix_embedding.norm(dim=-1, keepdim=True)
        else:
            # empty memory bank, use cls feature instead
            prefix_embedding = cls_features
        # prefix_embedding = cls_features

        embedding_clip = self.align_proj(prefix_embedding.to(torch.float32)).unsqueeze(1)
    ...
    def forward(self, clip_tokens, gpt_tokens):
        ...
        content_feature = clip_features.to(torch.float32)
